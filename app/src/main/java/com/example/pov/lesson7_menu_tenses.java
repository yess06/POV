...
public class lesson7_menu_tenses extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        }else if(valid.getInt("cont13", 0) == 1){
            btnVocabulary.setEnabled(false);
            btnPresent.setEnabled(true);
            btnFuture.setEnabled(false);
            btnQualification.setEnabled(false);
        }else if(valid.getInt("cont13", 0) == 2){
            btnVocabulary.setEnabled(false);
            btnPresent.setEnabled(false);
            btnFuture.setEnabled(true);
            btnQualification.setEnabled(false);
        }else if(valid.getInt("cont13", 0) == 3){
            btnVocabulary.setEnabled(false);
            btnPresent.setEnabled(false);
            btnFuture.setEnabled(false);
        ...
        btnFuture.setOnClickListener(new View.OnClickListener() {
         ...
         btnVocabulary.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(lesson7_menu_tenses.this, lesson7_menu_vocabulary.class);
                startActivity(intent);
            }
        });
         btnQualification.setOnClickListener(new View.OnClickListener() {
             @Override
             public void onClick(View view) {
                 Intent intent = new Intent(lesson7_menu_tenses.this, lesson7_qualification_student.class);
                 startActivity(intent);
             }
         });
    }
    ...
    public void getbtn() {
        ...
        try {
            progressDialog = new ProgressDialog(lesson7_menu_tenses.this);
                            ...
                            if (u.getString("user_id").equals(preferences.getString("id", "null"))){
                                if (u.getString("lesson_id").equals("7")){
                                    if (u.getString("time_id").equals("4")){
                                        ...
                                        editor.putInt("lesson7future", cont);
                                        editor.commit();
                                    }else if (u.getString("time_id").equals("3")){
                                        cont2++;
                                        SharedPreferences vali = getSharedPreferences("vali", Context.MODE_PRIVATE);
                                        SharedPreferences.Editor editor = vali.edit();
                                        editor.putInt("lesson7past", cont2);
                                        editor.commit();
                                    }
                                        ...
                                        SharedPreferences vali = getSharedPreferences("vali", Context.MODE_PRIVATE);
                                        SharedPreferences.Editor editor = vali.edit();
                                        editor.putInt("lesson7present", cont3);
                                        editor.commit();
                                    }
                                }
